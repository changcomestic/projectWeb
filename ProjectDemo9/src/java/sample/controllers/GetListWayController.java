 ...
 * @author sonho
 */
public class GetListWayController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<WayDTO> listWay = WayDAO.getListWayByIdCity(idCity);
            HttpSession session = request.getSession();
            session.setAttribute("LISTWAY", listWay);
            session.setAttribute("IDCITY", idCity);
        } catch (Exception ex) {
            log("Error at GetListWayController: " + ex.toString());
        } finally {
